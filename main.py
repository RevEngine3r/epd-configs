...
if __name__ == "__main__":
    ...
    pl.Path(sub_dir/'vvt.txt').write_text('\n'.join([l for l in pl.Path(sub_dir/'all.txt').read_text(encoding='utf-8').splitlines() if l.lower().startswith(('vless:',))]), encoding='utf-8')

    pl.Path(sub_dir/'vvt_new.txt').write_text('\n'.join([l for l in pl.Path(sub_dir/'all_new.txt').read_text(encoding='utf-8').splitlines() if l.lower().startswith(('vless:',))]), encoding='utf-8')
